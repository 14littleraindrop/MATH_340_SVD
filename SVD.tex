...
Singular Value Decomposition (SVD) is a factorization of a matrix into three separate matrices that give valuable properties. Let $A$ be an input matrix. Then we can decompose $A_{[m \times n]} = U_{[m \times r]} \Sigma_{[r \times r]} (V_{[n \times r]})^T$ where $U, \Sigma, V$ are unique, $U, V$ are orthonormal, and $\Sigma$ is diagonal. The singular values of $\Sigma$ are positive, and $\sigma_1 \geq \sigma_2 \geq \dots \geq 0$.

\subsection{Why is Singular Value Decomposition Useful?}
SVD is one of the most commonly used tools in data science today. The three matrices that SVD decomposes a matrix into allow us to understand and manipulate complex matrices with many data points. Since the singular values of $\Sigma$ are ordered, in large matrices we can use the first few dominant singular values, and ignore the rest, to arrive at an approximation of the input matrix. Below are several applications of SVD:
\begin{itemize}
  \item SVD can be used to solve $Ax = b$ where $A$ is not a square matrix. This approach is often used in linear regression models.
  \item We see SVD in Google's 'PageRank' algorithm, as well as in other modern search engines. It is also used to find correlation patterns for recommendations on Netflix, YouTube, etc.
  \item SVD is also used as a basis for Principal Component Analysis, which is key to interpreting data in higher dimensions and is used across machine learning.
\end{itemize}
